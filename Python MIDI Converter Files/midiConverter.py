import midi #MIDI library; https://github.com/vishnubob/python-midi
import os
...
class Song(object): #This class defines the behavior of a 'Song' object.
...
class MusicalNote(object): #Defines the behavior of a musical note.
    def __init__(self, pitch, tick):
        '''
        pitch: 'NoteOnEvents' in .midi files have a pitch that is an integer ranging from 0 to 127; each pitch corresponds to a particular frequency
        tick: the number of ticks that must pass after the previous event before the current event can occur
        '''
    ...
    def convertPitch(self, pitch):
        '''
        returns: the frequency corresponding to the given pitch
        '''
    ...
    def changeFrequency(self, newFrequency):
        '''
        function: makes it possible to directly alter the frequency of a particular note
        '''
